...
int myArray[21]={1,4,6,8,13,15,18,21,25,27,30,33,37,39,42,45,48,51,53,58,61};
     ...
     
{
  ...
  int number2=op[i][2];
    ...
    //(%lf,%lf),(%lf,%lf),(%lf,%lf)\n", op[i][0], op[i][1], op[i][2],
    //seg[number0].v0.x,seg[number0].v0.y,seg[number1].v0.x,seg[number1].v0.y,seg[number2].v0.x,seg[number2].v0.y);
    
  }
    fprintf(fp,"TOTAL NUMBER= %d", i);
  for (a=0;a<21;a++)
   {
     int value;
     value=myArray[a];

     printf("(%.30lf,%.30lf),", seg[value].v0.x,seg[value].v0.y);
   }
